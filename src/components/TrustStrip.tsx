      ...
      <section className="py-8 md:py-20 bg-background overflow-hidden">
        <div className="container mx-auto px-4 lg:px-8">
          <motion.div 
            ...
            viewport={{ once: true }}
            className="flex flex-col md:flex-row items-stretch rounded-2xl sm:rounded-3xl overflow-hidden relative shadow-[0_20px_50px_rgba(16,185,129,0.12)] border border-[#1a1a1a]"
          ...
          >
            ...
            {/* Left Side */}
            <div className="md:w-1/2 p-5 sm:p-8 md:p-16 relative z-10 flex flex-col justify-center" style={{ color: 'white' }}>
              <h2 className="text-2xl sm:text-3xl md:text-5xl font-extrabold italic mb-2 tracking-tight leading-tight" style={{ color: 'white' }}>
                open <span className="font-medium not-italic">to the progress of all</span>
              </h2>
              <p className="text-sm sm:text-lg mb-6 sm:mb-10" style={{ color: 'rgba(255,255,255,0.70)' }}>
                Committed to our responsibility towards the society.
              </p>
              
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-4 mb-6 sm:mb-10">
                {[
                  { val: 'Registered', label: 'Co-operative Society' },
                  { val: '100%', label: 'Regulated & Secured' },
                  { val: '25.00%', label: 'Max FD Interest Rate' },
                ].map((stat, i) => (
                  <div key={i} className="rounded-xl p-3 sm:p-5 text-center transition-all hover:bg-white/10 flex flex-col justify-center items-center" style={{ border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.04)' }}>
                    <p className="text-sm sm:text-lg lg:text-xl font-black mb-1 tracking-tight text-white" style={{ color: 'white' }}>{stat.val}</p>
                    <p className="text-[10px] sm:text-xs font-medium leading-tight" style={{ color: 'rgba(255,255,255,0.55)' }}>{stat.label}</p>
                  </div>
                ))}
              </div>
              ...
              <div>
                <button 
                  ...
                  className="bg-cyan-gradient text-black px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg text-xs sm:text-sm font-bold hover:opacity-90 transition-all shadow-[0_10px_20px_rgba(16,185,129,0.2)] hover:-translate-y-0.5 cursor-pointer"
                >
                  Know More
                </button>
              </div>
